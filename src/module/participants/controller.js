...
class Participants {
  static async showAllPollChose(req, res, next) {
    ...
    const allOptions = await OptionsReader.getAllOptionsByPollsListId(onePoll[0].id);
    onePoll[0].Options = allOptions

    const allParticipants = await ParticipantsReader.getAllparticipantsByPollId(onePoll[0].id);
